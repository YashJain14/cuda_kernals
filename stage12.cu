...
#include "stage12_include/common.h"
#include "stage12_include/flash_attention.cuh"
#include "stage12_include/forward_kernel.cuh"
#include "stage12_include/static_kernel_configuration.cuh"
...
using Config12 = flash::StaticForwardKernelConfig<cfg_stage12>;
...
extern "C"
void launch_flash_v12(const half* d_Q, const half* d_K, const half* d_V,
                      half* d_O, int B_nh, int N) {
    
    int n_Q_blocks = (N + 128 - 1) / 128;
    int n_KV_blocks = (N + 64 - 1) / 64;
    ...
    args.batch_stride = 0; // z dimension is 1
    args.head_stride = N * 64; // y dimension is B_nh
    args.seq_stride = 64;
    args.seq_len = N;
    args.n_heads = B_nh;
    ...
    size_t smem = cfg_stage12.smem_bytes();

    CUDA_CHECK(cudaFuncSetAttribute(flash::flash_forward_kernel<Config12>,
               cudaFuncAttributeMaxDynamicSharedMemorySize, smem));

    flash::flash_forward_kernel<Config12><<<grid, block, smem>>>(args);
}
// ═══════════════════════════════════════════════════════════════════════════════
...
int main() {
    ...
    for (int ci = 0; ci < 3; ci++) {
        ...
        for (int r=0;r<3;r++) launch_flash_v12(dQ,dK,dV,dO,B_nh,N);
        ...
        for (int r=0;r<10;r++) launch_flash_v12(dQ,dK,dV,dO,B_nh,N);
        ...
        printf("N=%d  Tc=%d\n", N, (N+BC-1)/BC);
        printf("  Stage 12: %7.3f ms  %6.2f TFLOPS  maxErr=%.2e  meanRelErr=%.2e  cosSim=%.6f\n\n",
