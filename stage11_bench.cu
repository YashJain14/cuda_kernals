...
using Config11 = flash::StaticForwardKernelConfig<cfg_stage11>;
...
int main() {
    ...
    // Print smem budget
    printf("============================================================\n");
    printf("  Stage 11  —  PTX from Scratch Repo\n");
    printf("  B=%d  nh=%d  d=%d  B_nh=%d  BR=%d  BC=%d\n", B,nh,d,B_nh,BR,BC);
    printf("  smem/block = %zu KB  (2 blocks → %zu KB < 164 KB)\n",
           cfg_stage11.smem_bytes()/1024, 2*cfg_stage11.smem_bytes()/1024);
    ...
    for (int ci = 0; ci < 3; ci++) {
        ...
        for (int r=0;r<10;r++) launch_flash_v11(dQ,dK,dV,dO,B_nh,N);
        ...
        for (int r=0;r<100;r++) launch_flash_v11(dQ,dK,dV,dO,B_nh,N);
        ...
        printf("N=%d  Tc=%d\n", N, (N+BC-1)/BC);
        printf("  Stage 11: %7.3f ms  %6.2f TFLOPS  maxErr=%.2e  meanRelErr=%.2e  cosSim=%.6f\n\n",
