...
// ── Launcher ──
extern "C"
void launch_flash_v9(const half* d_Q, const half* d_K, const half* d_V, half* d_O, int B_nh, int N) {
    dim3 grid((N + BR - 1) / BR, B_nh);
    dim3 block(BLK);
    size_t smem = sizeof(SharedStorage);
    CUDA_CHECK(cudaFuncSetAttribute(flash_v9_cute_kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
...
int main(int argc, char** argv) {
    ...
    printf("============================================================\n");
    printf("  Stage 9 (CuTe Optimized) Benchmark\n");
    ...
    for (int ci = 0; ci < 3; ci++) {
        ...
        for (int r = 0; r < warmup; r++) launch_flash_v9(d_Q, d_K, d_V, d_O, B_nh, N);
        CUDA_CHECK(cudaDeviceSynchronize());

        CUDA_CHECK(cudaMemset(d_O, 0, qkv_half_bytes));
        CUDA_CHECK(cudaEventRecord(start));
        for (int r = 0; r < iters; r++) launch_flash_v9(d_Q, d_K, d_V, d_O, B_nh, N);
        ...
        printf("N = %d\n", N);
        printf("  Stage 9 (CuTe Fixed): %8.3f ms  %6.2f TFLOPS  maxErr=%.2e  meanRelErr=%.2e  cosSim=%.6f\n\n", ms, tflops, mae, mre, cossim);
