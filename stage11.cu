...
// ── Configuration ──
// Stage 11 diagnosis: BC=128 required 112 KB smem → only 1 block/SM → 8 warps/SM.
...
flash_v11_cute_kernel(const half_t* __restrict__ gQ_ptr,
...
void launch_flash_v11(const half* d_Q, const half* d_K, const half* d_V,
    ...
    CUDA_CHECK(cudaFuncSetAttribute(flash_v11_cute_kernel,
               cudaFuncAttributeMaxDynamicSharedMemorySize, smem));
    flash_v11_cute_kernel<<<grid, block, smem>>>(
...
int main() {
    ...
    // Print smem budget
    printf("============================================================\n");
    printf("  Stage 11  —  ldmatrix + BC=64 + 2 blocks/SM\n");
